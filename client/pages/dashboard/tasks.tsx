...
function TasksContent() {
    ...
    const handleCreateTask = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post(`${API_URL}/api/tasks`, payload, { withCredentials: true });
    ...
    const toggleComplete = async (id: string, current: boolean) => {
        try {
            const res = await axios.put(`${API_URL}/api/tasks/${id}`, { completed: !current }, { withCredentials: true });
        ...
        <div className="max-w-7xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div>
                    ...
                    <p className="text-slate-500 font-medium font-outfit">Manage your time and focus efficiently.</p>
                </div>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-center">
                    ...
                    <div className="flex w-full h-4 rounded-full overflow-hidden mb-2">
                        <div style={{ width: `${(tasks.filter(t => t.priority === 'high').length / (tasks.length || 1)) * 100}%` }} className="bg-red-500 h-full" />
                        <div style={{ width: `${(tasks.filter(t => t.priority === 'medium').length / (tasks.length || 1)) * 100}%` }} className="bg-orange-400 h-full" />
                        <div style={{ width: `${(tasks.filter(t => t.priority === 'low').length / (tasks.length || 1)) * 100}%` }} className="bg-blue-400 h-full" />
                    </div>
                    ...
                    <div className="space-y-4 relative pl-4 border-l-2 border-slate-100 dark:border-slate-800">
                        {visibleScheduled.length === 0 && (
                            <div className="p-8 text-center text-slate-400 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 ml-4">
                                ...
                                <p className="font-outfit">No scheduled tasks found.</p>
                            </div>
                        )}
                ...
                {showModal && (
                    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <form onSubmit={handleCreateTask} className="p-8 space-y-6 text-slate-900 dark:text-white">
                                {/* Type Toggle */}
                                <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
                                    <button
                                        type="button"
                                        onClick={() => setNewTask({ ...newTask, isTodo: false })}
                                        className={cn("flex-1 py-2 rounded-lg text-sm font-bold transition", !newTask.isTodo ? "bg-white dark:bg-slate-700 shadow text-indigo-600" : "text-slate-500")}
                                    >
                                        Scheduled Task
                                    </button>
                                    <button
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1 font-outfit">Title</label>
                                        <input
                                            autoFocus
                                            className="w-full mt-1 p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white"
                                    ...
                                    {!newTask.isTodo && (
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1 font-outfit">Start Time</label>
                                                <input
                                                    type="datetime-local"
                                                    className="w-full mt-1 p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white"
                                                    value={newTask.startTime}
                                                    onChange={e => setNewTask({ ...newTask, startTime: e.target.value })}
                                                />
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1 font-outfit">Priority</label>
                                                <select
                                                    className="w-full mt-1 p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white"
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider pl-1 font-outfit">Subject / Tag</label>
                                        <input
                                            className="w-full mt-1 p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white"
